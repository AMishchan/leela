...
class Player(models.Model):
    ...
    telegram_username = models.CharField('Никнейм в Telegram', max_length=150, blank=True)
    ...
    main_status = models.CharField('Главный статус', max_length=16,
                                   choices=MainStatus.choices, default=MainStatus.ACTIVE)
    payment_status = models.CharField('Статус оплаты', max_length=16,
                                      choices=PaymentStatus.choices, default=PaymentStatus.NONE)
    player_type = models.CharField('Тип игрока', max_length=16,
                                   choices=PlayerType.choices, default=PlayerType.FREE)

    # игра (пока просто полями; позже можно вынести в отдельную модель Game)
